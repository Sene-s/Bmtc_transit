...
# VERSION BUMP: Clears the corrupted SID cache from Render
CACHE_FILE = "graph_cache_v_FINAL_STABLE_v2.msgpack"
TRANSFER_PENALTY = 10
...
class TransitEngine:
    ...
    async def initialize(self):  
        ...
        if os.path.exists(CACHE_FILE):  
            try: 
                self._load_cache()
                if self.G.vcount() == 0 or self.G.vs[0]["sid"] is None:
                    raise ValueError("Graph attributes missing in cache")
                log.info("📦 Graph loaded successfully from cache.")
            except Exception as e: 
                log.warning(f"Cache failed ({e}). Rebuilding...")
                await self._build(); self._save_cache()
        else: 
            await self._build(); self._save_cache()
        self._build_name_index()

    async def _build(self):  
        log.info("🏗️ Fetching stops from Supabase...")
        offset = 0  
        while True:  
        ...
        # Initialize Graph Cleanly
        sids = list(self.stop_details.keys())
        self.G = ig.Graph(directed=True)
        self.G.add_vertices(len(sids)) # MUST ADD VERTICES FIRST
        self.G.vs["sid"] = sids        # THEN ASSIGN ATTRIBUTES
        self._node_index = {sid: i for i, sid in enumerate(sids)}

        log.info("🏗️ Fetching edges from Supabase...")
        offset = 0
        while True:
    ...
    def _add_walking_mesh(self):
        ...
        for i, (coord, g_idx) in enumerate(valid):
            dists, idxs = tree.query(coord, k=10) # Higher K for better connectivity
    ...
    async def find_route(self, start: str, end: str):
        ...
        while queue:
            ...
            for eid in self.G.incident(u, mode="out"):
                ...
                sid_v = self.G.vs[v]["sid"]
                
                if sid_v is None or sid_e is None or sid_v not in self.stop_details: continue

                new_c = cost + w + (TRANSFER_PENALTY if edge["type"] == "bus" and prev_r and r != prev_r else 0)
                h = haversine_dist((self.stop_details[sid_v]['lat'], self.stop_details[sid_v]['lng']), 
                                   (self.stop_details[sid_e]['lat'], self.stop_details[sid_e]['lng'])) / 0.8
    ...
    def _load_cache(self):
        ...
        self.G = ig.Graph(directed=True)
        self.G.add_vertices(len(data["sids"])) # FIXED ORDER
